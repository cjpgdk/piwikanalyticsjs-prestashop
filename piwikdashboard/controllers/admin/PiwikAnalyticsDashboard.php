...
class PiwikAnalyticsDashboardController extends ModuleAdminController {
    ...
    public function initContent() {
        ...
        $this->toolbar_title = $this->l('Stats', 'PiwikAnalyticsDashboard');
        ...
        if ($this->display == 'view') {
            ...
            if ((empty($PIWIK_HOST) || $PIWIK_HOST === FALSE) ||
                ...
                $this->content .= <<< EOF
<script type="text/javascript">function WidgetizeiframeDashboardLoaded() { var w = $('#content').width(); var h = $('body').height(); $('#WidgetizeiframeDashboard').width('100%'); $('#WidgetizeiframeDashboard').height(h); }</script>   
EOF;
                $lng = new LanguageCore($this->context->cookie->id_lang);

                if (_PS_VERSION_ < '1.6')
                    $this->content .= '<h3><a target="_blank" href="' . $this->page_header_toolbar_btn['stats']['href'] . '">' . $this->page_header_toolbar_btn['stats']['desc'] . '</a> | <a target="_blank" href="https://github.com/cmjnisse/piwikdashboard-prestashop/wiki">' . $this->l('Help') . '</a></h3>';
